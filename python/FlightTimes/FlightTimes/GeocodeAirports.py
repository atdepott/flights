...
input_locations = os.path.join("data/airports.csv")
...
def write_city(out_file_name, location, lat_long, airport_code, airport_name):
    lat = ""
    long = ""
    if (lat_long != ""):
        lat = lat_long["lat"]
    ...
    writer.writerow([location,str(lat),str(long), airport_code, airport_name]);
    out_file.close() 
...
with open(input_locations) as location_file:
    ...
    for row in reader:
        if ((rowNum >= input_line_start) and (rowNum < num_geocodes + input_line_start)):
            ...
            write_city(out_file_name, location, geocode_results, row[1], row[4]);
            time.sleep(3)
        rowNum = rowNum + 1